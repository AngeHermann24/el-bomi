...
export default function EnergieProjetsClient() {
  ...
  return (
    <div className="bg-gradient-to-b from-[#BFB499] via-[#C9BFA8] to-[#BFB499] text-navy-900">
      ...
      {/* Filtres sticky */}
      <section className="sticky top-[72px] z-30 border-b border-gold-500/15 bg-[#BFB499]/95 backdrop-blur py-4">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`rounded-full px-4 py-1.5 text-sm font-semibold transition-all duration-300 ${
                  active === cat
                    ? 'bg-gradient-to-r from-gold-600 to-amber-accent text-navy-900 shadow-[0_0_15px_rgba(242,169,59,0.2)]'
                    : 'border border-navy-900/10 text-navy-900/50 hover:border-amber-accent/30 hover:text-navy-900'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
            {filtered.map((p, i) => (
              <motion.div
                ...
                className="group relative overflow-hidden rounded-2xl border border-navy-900/8 bg-white/40 transition-all duration-300 hover:border-amber-accent/25 hover:shadow-[0_0_20px_rgba(242,169,59,0.08)]"
              >
                {/* Image */}
                <div className="relative aspect-[16/10] overflow-hidden">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#B5AB94]/80 via-[#B5AB94]/20 to-transparent" />
                  {/* Badge catégorie */}
                  <div className="absolute left-3 top-3">
                    <span className="rounded-full border border-gold-500/30 bg-white/75 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-gold-600 backdrop-blur-sm">
                      {p.category}
                    </span>
                  </div>
                  {/* Glow hover */}
                  <div className="absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100"
                    style={{ background: 'radial-gradient(ellipse at 50% 100%, rgba(242,169,59,0.12) 0%, transparent 70%)' }}
                  />
                </div>
                ...
                <div className="p-6">
                  <div className="mb-3 flex flex-wrap gap-1.5">
                    {p.tags.map((t) => (
                      <span key={t} className="rounded-full border border-gold-500/20 bg-gold-500/8 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-gold-600/70">
                        {t}
                      </span>
                    ))}
                  </div>
                  <h3 className="mb-2 font-heading text-lg font-bold uppercase text-navy-900">{p.title}</h3>
                  <p className="mb-4 text-sm leading-relaxed text-navy-900/50">{p.desc}</p>
                  <div className="flex flex-wrap gap-4 text-xs text-navy-900/40">
                    <span className="flex items-center gap-1.5">
                      <MapPin className="h-3.5 w-3.5 text-gold-600/50" />{p.location}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Calendar className="h-3.5 w-3.5 text-gold-600/50" />{p.year}
                    </span>
                    <span className="ml-auto font-semibold text-amber-accent/70">{p.scope}</span>
                  </div>
                </div>
                {/* Barre or bas */}
      ...
      <section className="bg-gradient-to-b from-[#B5AB94] to-navy-900 py-12 sm:py-20 text-center">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="mx-auto mt-4 max-w-lg text-white/50">Contactez-nous pour un devis personnalisé.</p>
