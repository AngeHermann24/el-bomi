  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${
      ...
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        {/* Logo */}
        <Link href="/filiales/medical" className="group flex items-center gap-3">
          <div className="relative flex h-9 w-9 items-center justify-center rounded-xl border border-[#4A7C7C]/20 bg-[#4A7C7C]/8 shadow-sm transition-all group-hover:bg-[#4A7C7C]/14">
            <HeartPulse className="h-4.5 w-4.5 text-[#4A7C7C]" strokeWidth={1.8} />
          </div>
          <div>
            <span className="block font-heading text-[11px] font-bold uppercase tracking-[0.25em] text-[#0B1E3D]">EL-BOMI</span>
            <span className="block font-heading text-[10px] uppercase tracking-widest text-[#0B1E3D]/35">Medical Distribution</span>
          </div>
        </Link>
        ...
        <button onClick={() => setOpen(!open)} aria-label="Menu"
          className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#0B1E3D]/10 text-[#0B1E3D]/40 md:hidden"
        >
            ...
            <nav className="flex flex-col px-4 py-4">
              {nav.map((item) => (
                <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
                  className="border-b border-[#0B1E3D]/[0.05] py-3.5 text-sm font-medium text-[#0B1E3D]/50 hover:text-[#0B1E3D]"
                >
                  {item.label}
                </Link>
              ))}
