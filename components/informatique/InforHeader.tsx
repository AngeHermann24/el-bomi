  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'border-b border-gold-500/10 bg-[#0B1E3D]/95 shadow-[0_4px_30px_rgba(11,30,61,0.5)] backdrop-blur-md'
          : 'bg-transparent'
      }`}
    >
      {/* Ligne scan top */}
      ...
      <div className="mx-auto flex h-[70px] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        {/* Logo */}
        <Link href="/filiales/informatique-telecoms" className="group flex items-center gap-3">
          <div className="relative flex h-9 w-9 items-center justify-center">
            <div className="absolute inset-0 rounded-lg bg-gold-500/10 blur-md transition-all group-hover:bg-gold-500/25 group-hover:blur-lg" />
          ...
          <div>
            ...
            <span className="block font-heading text-[10px] uppercase tracking-[0.15em] text-white/35">Informatique & Télécoms</span>
          </div>
        </Link>
        ...
        <nav className="hidden items-center gap-0.5 md:flex">
          ...
          <Link
            ...
            className="group relative ml-3 overflow-hidden rounded-lg border border-gold-500/25 bg-gold-500/8 px-4 py-2 text-[13px] font-semibold text-gold-300 transition-all duration-300 hover:bg-gold-500/15 hover:border-gold-400/40 hover:shadow-[0_0_15px_rgba(201,162,39,0.15)]"
          >
            ...
            <nav className="flex flex-col px-4 py-4">
              {nav.map((item) => (
                <Link
                  ...
                  className="border-b border-white/[0.05] py-3 text-sm font-medium text-white/55 transition-colors hover:text-gold-400"
                >
                  {item.label}
                </Link>
              ))}
