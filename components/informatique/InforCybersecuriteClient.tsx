...
export default function InforCybersecuriteClient() {
  return (
    <div className="bg-[#D6E4F0] text-navy-900">
      ...
      <section className="relative min-h-[60vh] flex items-end overflow-hidden bg-white">
        <div className="absolute inset-0">
          <Image
            ...
            fill priority className="object-cover opacity-25"
            sizes="100vw"
          />
        ...
        <div className="container mx-auto relative px-4 sm:px-6 lg:px-8 pb-16">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="mb-5 flex items-center gap-3">
            ...
            <span className="text-xs font-semibold uppercase tracking-[0.3em] text-gold-600">Sécurité IT</span>
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            className="font-heading text-5xl font-bold uppercase text-navy-900 md:text-6xl lg:text-7xl"
          >
            Cybersécurité &<br />
            <span className="bg-gradient-to-r from-gold-600 to-gold-500 bg-clip-text text-transparent">protection totale</span>
          </motion.h1>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
            className="mt-6 inline-flex items-center gap-2 rounded-lg border border-green-500/25 bg-white/85 px-4 py-2 backdrop-blur-sm shadow-sm"
          >
            <span className="relative flex h-2 w-2">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-green-500 opacity-75" />
              <span className="relative inline-flex h-2 w-2 rounded-full bg-green-500" />
            </span>
            <span className="text-xs font-semibold text-green-600">SOC opérationnel — surveillance active</span>
          </motion.div>
        </div>
      </section>

      {/* Stats */}
      <section className="bg-[#C8D8E8] py-10 sm:py-16">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {stats.map((s, i) => (
              <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                className="group relative rounded-xl border border-navy-900/8 bg-white p-5 shadow-sm transition-all hover:border-gold-500/25 hover:shadow-md"
              >
                <div className="absolute top-0 right-0 h-5 w-5 border-r border-t border-gold-500/30" />
                <p className="font-heading text-2xl font-bold text-gold-600 md:text-3xl">{s.value}</p>
                <p className="mt-1 text-sm text-navy-900/70">{s.label}</p>
          ...
          <div className="grid grid-cols-1 gap-8 sm:gap-12 lg:grid-cols-2 lg:items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="mb-5 flex items-center gap-3">
                <span className="h-px w-8 bg-gold-600" />
                <span className="text-xs font-semibold uppercase tracking-[0.3em] text-gold-600">Threat landscape</span>
              </div>
              <h2 className="font-heading text-3xl font-bold uppercase text-navy-900 md:text-4xl">
                Les menaces sont réelles,<br />
                <span className="text-gold-600">nos défenses aussi</span>
              </h2>
              <p className="mt-5 text-navy-900/55 leading-relaxed">
                ...
                {threats.map((t, i) => (
                  <motion.div key={t.label} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                    <div className="mb-1.5 flex justify-between text-sm">
                      <span className="text-navy-900/60">{t.label}</span>
                      <span className="font-semibold text-gold-600/80">+{t.pct}%</span>
                    </div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
              className="relative overflow-hidden rounded-xl border border-navy-900/10 shadow-md" style={{ minHeight: '360px' }}
            >
              <Image src="/images/infor-projets.jpg" alt="SOC" fill
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-6 space-y-2">
                {[
                  ...
                  <div key={row.label} className="flex items-center justify-between rounded-lg border border-white/30 bg-white/85 px-4 py-2.5 backdrop-blur-sm">
                    <span className="text-xs text-navy-900/55">{row.label}</span>
                    <span className={`font-heading text-sm font-bold ${row.ok ? 'text-green-600' : 'text-red-500'}`}>{row.val}</span>
                  </div>
                ))}
              </div>
            </motion.div>
      ...
      {/* Services grille */}
      <section className="bg-[#C8D8E8] py-12 sm:py-20">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12 text-center">
            <div className="mb-4 flex items-center justify-center gap-3">
              <span className="h-px w-8 bg-gold-600" />
              <span className="text-xs font-semibold uppercase tracking-[0.3em] text-gold-600">Nos services</span>
              <span className="h-px w-8 bg-gold-600" />
            </div>
            <h2 className="font-heading text-3xl font-bold uppercase text-navy-900 md:text-4xl">Protection complète</h2>
          </motion.div>
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
            {services.map((s, i) => (
              <motion.div key={s.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                className="group relative rounded-xl border border-navy-900/8 bg-white p-6 shadow-sm transition-all hover:border-gold-500/25 hover:shadow-md"
              >
                ...
                <h3 className="mb-2 font-heading text-sm font-bold uppercase text-navy-900 leading-snug">{s.title}</h3>
                <p className="mb-4 text-xs leading-relaxed text-navy-900/50">{s.desc}</p>
                <ul className="space-y-1.5">
                  {s.items.map((item) => (
                    <li key={item} className="flex items-center gap-2 text-xs text-navy-900/45">
                      <CheckCircle2 className="h-3.5 w-3.5 shrink-0 text-cyan-accent" /> {item}
                    </li>
                  ))}
                </ul>
      ...
      <section className="bg-white py-12 sm:py-20 text-center">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="mx-auto mt-4 max-w-lg text-navy-900/45">Notre équipe réalise un diagnostic de premier niveau sans engagement.</p>
